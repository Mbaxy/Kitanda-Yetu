...
class RegisterProducts(UserControl):
    def __init__(self, route):
        ...
        self.tf_costs = TextField(label="Custo", expand=2, prefix_text="AOA", text_size=24, dense=True,  on_change=self.analyze_fields, on_blur=self.calc_margin)
        self.tf_selling_price = TextField(label="Preço de Venda", expand=2, prefix_text="AOA", text_size=24, dense=True,  on_change=self.analyze_fields, on_blur=self.calc_margin)
        ...
        self.text_total = Text(value="AOA 350,00", style=TextThemeStyle.TITLE_MEDIUM)
    ...
    def calc_selling_price(self, e):
        ...
        if cost != 0:
            ...
            Notification(self.route.page, "Não é possível calcular o preço de venda com base na margem, pois o custo é AOA 0,00!", "red").show_message()
            return
    ...
    def get_sold_from_db(self):
        ...
        self.text_total.value = f"AOA {Validator.format_to_currency(total)}"
        self.update()
            ...
            self.dt_order_history.rows.append(
                DataRow(
                    cells=[
                        ...
                        DataCell(Text(value=f"AOA {Validator.format_to_currency(data[2])}")),
